...
impl Header {
    ...
    pub async fn get_all_msg(&self) -> anyhow::Result<Vec<RecMessage>> {
        let msgs =sqlx::query_as(
            r#"select m.id,m.topic,m.qos,m.retain,m.mine,m.timestamp,m.sender,m.receiver,m.type,m.raw,m.html from message m where m.header_id = ?"#
        ).bind(&self.id)
        .fetch_all(&*DB_POOL)
        .await?;
    ...
    pub async fn get_db_header() -> anyhow::Result<Vec<Header>> {
