...
pub fn backend<Store: SessionStore>(
    ...
    Router::new()
        .merge(back_auth_route(send_msg_tx.clone(), rec_msg_tx.clone()))
        .merge(back_token_route(shared_state, send_msg_tx.clone()))
...
fn back_auth_route(
    ...
    Router::new()
        ...
        .nest("/c", back_chat_route_c(send_msg_tx, rec_msg_tx))
        .route_layer(middleware::from_fn(middlewares::user_secure))
}

fn back_token_route<S>(state: Arc<Store>, send_msg_tx: UnboundedSender<SendMessage>) -> Router<S> {
    Router::new()
        .route("/check", get(routes::api_handler))
        .nest("/a", back_chat_route_a(state.clone(), send_msg_tx))
...
fn back_chat_route<S>(state: S, send_msg_tx: UnboundedSender<SendMessage>) -> Router<S>
where
...
fn back_chat_route_c(
    ...
    Router::new()
        .merge(back_chat_route((), send_msg_tx))
        .merge(script_file())
...
fn back_chat_route_a<S>(state: Arc<Store>, send_msg_tx: UnboundedSender<SendMessage>) -> Router<S> {
    Router::new()
        .merge(back_chat_route(state.clone(), send_msg_tx))
        .with_state(state)
}
